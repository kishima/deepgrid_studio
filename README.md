...
| `G` / `B` / `C` | 拾う / 防ぐ / 精神統一 |
| `T` / `V` | 投げる / 盗む |
| `Tab` / `I` | データ画面(持ち物・装備・セーブ)。`M` は魔法タブ直行 |
| `O` | プレイ中のキー割当変更 |

マウスだけでも遊べる: 左下の移動パッド(↑↓←→・UP/DOWN)と右下の
アクションボタン(攻撃/防ぐ/精神/投げる/盗む/取る/魔法)が同じ操作に対応する。
...
```sh
./docker/deepgrid-run.sh --edit             # エディターで起動
```

通常起動(タイトル画面)からも「エディター」項目で入れる。エディター・タイトル・
プレイはすべて **1プロセス・1ウィンドウ**で、ウィンドウを閉じずに行き来できる
(配布版 `play_only` ではタイトルに「エディター」は出ない)。
...
上部バーの「▶ テストプレイ (F5)」または **F5** で、**未保存の編集内容のまま**
その場でプレイに切り替わる。開始地点から歩けて、ダンジョンはメモリ上の編集を
反映する(ディスクへは保存しない使い捨ての世界)。もう一度 **F5** でエディターに
戻り、Undo履歴・タブ・選択はそのまま残っている。テストプレイ中はデータ画面の
セーブ/ロードが無効(未保存プロジェクトとセーブが食い違うため)。「タイトルへ」で
編集後の内容を反映したタイトル画面へ戻る。
...
### 画面遷移(GameScreen States, plan12/13)

最上位画面は Bevy の `States`([src/screen.rs](src/screen.rs) の
`GameScreen`: `Title` / `Demo` / `Playing` / `Editor`)。遷移は
`NextState<GameScreen>`、各画面の生成・破棄は `OnEnter` / `OnExit`、
画面依存のシステムは `run_if(in_state(...))` または関数内で
`Res<State<GameScreen>>` を読む。

- 起動時の初期状態は `insert_state` で選ぶ(通常は `Title`、autotest /
  `--load` / 各 debug-shot(`title` を除く)/ perf は `Playing` 直行、
  `--edit` と `editor-*` シーンは `Editor`)。
- **Editor 状態**(plan13): プレイ側の Update 群は `screen::not_editor` で
  停止し、プレイの見た目(カメラ/HUD/ポートレート=`PlayScoped`)は出入りで
  完全再構築する。Project→ランタイムリソースの導出は
  [src/runtime.rs](src/runtime.rs) の `build_runtime` に一元化されており、
  起動・テストプレイ・エディター→タイトルはすべて同じ関数を通る。
...
- 優先則 Title > Demo > Editor > Data > Play は `screen::active_screen` に
  一元化。クロック凍結・移動凍結・キー設定はこの判定(または State 参照)を見る。
...
DEEPGRID_AUTOTEST=1 ./docker/deepgrid-run.sh          # 無人受け入れテスト(52ステップ)
DEEPGRID_DEBUG_SHOT=<scene> ./docker/deepgrid-run.sh  # debug-shot.png を出力し終了
```

> **注意**: autotest の合否は標準出力の `[autotest] ALL PASS` / `FAIL` 行で
> 判定すること。`App::run()` の `AppExit::Error` がプロセス終了コードに
> 伝播しない既知の事象があり、失敗時も exit 0 になりうる(要改善)。
...
`editor-testplay`(エディターで置いた壁を F5 でテストプレイし一人称視点を撮影、plan13)。
