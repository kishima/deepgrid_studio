...
- タイトルは DemoState と同じ「リソースでゲートして各システムが見る」
  方式で作る(States への全面移行は plan12 で独立実施と決定済み —
  機能 plan に混ぜない)。**その移行を見越し、「今どの画面がアクティブか」
  の判定(タイトル/デモ/データ画面/プレイ)は1つのヘルパー関数に集約**し、
  各システムが個別リソースを直接見る箇所をこれ以上増やさないこと。
  plan12 はこのヘルパーを States に差し替えるだけで済む形が理想。
