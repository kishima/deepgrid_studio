...
`MagicKind`(src/magic.rs)の実装状況。オリジナル27種のうち23種を実装済み、
内訳不明の4種は未実装(判明したらこの表と enum を更新する)。

| 種別 | 数 | 状態 | 備考 |
| --- | --- | --- | --- |
| `StatChange(StatKind)` | 15 | 実装済 | 能力値15種。`ActiveEffect` 枠(同一id重ねがけは持続リセット) |
| `HpChange` | 1 | 実装済 | 栄養価枠。hunger有効時は満腹度にも作用。攻撃魔法は `|value|` |
| `MpChange` | 1 | 実装済 | 0〜max_mp にクランプ |
| `Revive{ratio_percent}` | 3 | 実装済 | 33/50/100%(データ値。実装は任意%対応) |
| `Light{strength}` | 3 | 実装済 | 弱×1.5 / 中×2.5 / 強×4.0、`LightBoost` で持続減衰 |
| (内訳不明) | 4 | 未実装 | 判明後に enum へ追加し本表を更新 |

補足: 攻撃魔法(`projectiles>0`)・秘薬(液体化/飲む/投げつける)・巻物習得は
上記種別に直交する仕組みとして実装済み。
